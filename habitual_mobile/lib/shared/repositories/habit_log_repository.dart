...
class HabitLogRepository {
  ...
  Future<HabitLog?> getLogByHabitAndDate(int habitId, DateTime date) async {
    ...
    final logs = _databaseService.habitLogsBox.values.toList();
    print('DEBUG: Searching through ${logs.length} logs for habitId: $habitId on date: ${startOfDay.toString()}');
    ...
    for (final log in habitLogs) {
      ...
      print('DEBUG: Comparing log date: $logDate with target date: $targetDate');
      
      if (logDate.isAtSameMomentAs(targetDate)) {
        print('DEBUG: Found matching log for habitId: $habitId on date: $targetDate');
        return log;
      }
    }
    print('DEBUG: No matching log found for habitId: $habitId on date: $startOfDay');
    return null;
  }
  ...
  Future<int> calculateStreak(int habitId) async {
    ...
    bool completedToday = logs.any((log) => 
        log.dateOnly.isAtSameMomentAs(checkDate));

    if (!completedToday) {
      // Check if completed yesterday
      checkDate = checkDate.subtract(const Duration(days: 1));
      bool completedYesterday = logs.any((log) => 
          log.dateOnly.isAtSameMomentAs(checkDate));
    ...
    for (final log in logs) {
      ...
      if (habit != null) {
        ...
        if (category != null) {
          ...
          print('DEBUG getCompletionStatsByCategory: Log for habit "${habit.title}" -> Category "${categoryName}"');
        } else {
          // Fallback for habits without valid category
          stats['Tanpa Kategori'] = (stats['Tanpa Kategori'] ?? 0) + 1;
          print('DEBUG getCompletionStatsByCategory: Log for habit "${habit.title}" -> No valid category (categoryId: ${habit.categoryId})');
        }
      } else {
        print('DEBUG getCompletionStatsByCategory: No habit found for log with habitId: ${log.habitId}');
      }
    }
  ...
  Future<List<HabitLog>> getLogsInDateRange(DateTime start, DateTime end) async {
    await _databaseService.initialize();
    final logs = _databaseService.habitLogsBox.values.toList();
